...
from parse_json import tls_annotations_json, qc_annotations_json
...
def _create_legend(ax, qc_color_mapping):
    """
    Create a legend showing TLS and QC annotation colors.
    
    Args:
        ax: Matplotlib axes to draw the legend on.
        qc_color_mapping (dict): Mapping from QC type name to color string.
    """
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis('off')
    ...
    ax.text(0.5, 0.95, 'Legend', fontsize=30, fontweight='bold', ha='center', va='top')
    
    # TLS legend entry (green outline, no fill)
    tls_y = 0.74
    ax.add_patch(plt.Rectangle((0.1, tls_y - 0.02), 0.08, 0.04, 
                              fill=False, edgecolor='lime', linewidth=3))
    ax.text(0.25, tls_y, 'TLS', fontsize=24, va='center')
    
    # QC legend entries
    qc_entries = list(qc_color_mapping.items())
    y_start = 0.62
    y_spacing = 0.12
    
    for i, (qc_type, color) in enumerate(qc_entries):
        y_pos = y_start - i * y_spacing
        if y_pos < 0.05:  # Stop if we run out of space
            break
            
        # Colored rectangle for QC type
        ax.add_patch(plt.Rectangle((0.1, y_pos - 0.02), 0.08, 0.04, 
                                  facecolor=color, alpha=0.7, edgecolor=color))
        ax.text(0.25, y_pos, qc_type, fontsize=24, va='center')

def display_annotations_tls_qc(qc_annotations, tls_annotations, image_path, save_path=None, with_mask=True, 
    ...
    # Add 1 column for the legend
    num_cols = num_per_row + 1
    num_rows = math.ceil(num_tls / num_per_row)
    fig, axes = plt.subplots(num_rows, num_cols, figsize=(num_cols * 4, num_rows * 4))
    axes = axes.flatten() if num_tls > 1 or num_cols > 1 else [axes]
    
    # Process each TLS annotation
    for i in range(num_tls):
        current_polygon = all_tls_polygons[i]
        
        # Calculate the position in the grid, accounting for the legend column
        row = i // num_per_row
        col = i % num_per_row
    ...
    # Add legend to the rightmost column of the first row
    legend_ax_index = num_per_row  # This is the rightmost column of the first row
    legend_ax = axes[legend_ax_index]
    _create_legend(legend_ax, QC_COLOR_MAPPING)

    # Turn off any extra axes (skip the legend axis)
    for j in range(len(axes)):
        if j == legend_ax_index:  # Skip the legend axis
            continue
        # Calculate if this axis should be used for TLS data
        row = j // num_cols
        col = j % num_cols
        tls_index = row * num_per_row + col
        if col >= num_per_row or tls_index >= num_tls:
            axes[j].axis('off')
